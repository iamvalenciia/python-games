...
from game.casting.helicopter import Helicopter
...
from game.scripting.collide_helicopter_action import CollideHelicopterAction
from game.scripting.control_helicopter_action import ControlHelicopterAction
...
from game.scripting.draw_helicopter_action import DrawHelicopterAction
from game.scripting.end_drawing_action import EndDrawingAction
from game.scripting.initialize_devices_action import InitializeDevicesAction
from game.scripting.load_assets_action import LoadAssetsAction
from game.scripting.move_ball_action import MoveBallAction
from game.scripting.move_helicopter_action import MoveHelicopterAction
...
class SceneManager:
    ...
    COLLIDE_HELICOPTER_ACTION = CollideHelicopterAction(PHYSICS_SERVICE, AUDIO_SERVICE)
    CONTROL_HELICOPTER_ACTION = ControlHelicopterAction(KEYBOARD_SERVICE)
    DRAW_BALL_ACTION = DrawBallAction(VIDEO_SERVICE)
    DRAW_BRICKS_ACTION = DrawBricksAction(VIDEO_SERVICE)
    DRAW_DIALOG_ACTION = DrawDialogAction(VIDEO_SERVICE)
    DRAW_HUD_ACTION = DrawHudAction(VIDEO_SERVICE)
    DRAW_HELICOPTER_ACTION= DrawHelicopterAction(VIDEO_SERVICE)
    END_DRAWING_ACTION = EndDrawingAction(VIDEO_SERVICE)
    INITIALIZE_DEVICES_ACTION = InitializeDevicesAction(AUDIO_SERVICE, VIDEO_SERVICE)
    LOAD_ASSETS_ACTION = LoadAssetsAction(AUDIO_SERVICE, VIDEO_SERVICE)
    MOVE_BALL_ACTION = MoveBallAction()
    MOVE_HELICOPTER_ACTION = MoveHelicopterAction()
    ...
    def _prepare_in_play(self, cast, script):
        ...
        script.clear_actions(INPUT)
        script.add_action(INPUT, self.CONTROL_HELICOPTER_ACTION)
        self._add_update_script(script)
        self._add_output_script(script)
    ...
    def _add_ball(self, cast):
        ...
        y = SCREEN_HEIGHT - HELICOPTER_HEIGHT - BALL_HEIGHT  
        position = Point(x, y)
    ...
    def _add_helicopter(self, cast):
        cast.clear_actors(HELICOPTER_GROUP)
        x = CENTER_X - HELICOPTER_WIDTH / 2
        y = SCREEN_HEIGHT - HELICOPTER_HEIGHT
        position = Point(x, y)
        size = Point(HELICOPTER_WIDTH, HELICOPTER_HEIGHT)
        velocity = Point(0, 0)
        body = Body(position, size, velocity)
        animation = Animation(HELICOPTER_IMAGES, HELICOPTER_RATE)
        helicopter = Helicopter(body, animation)
        cast.add_actor(HELICOPTER_GROUP, helicopter)
    ...
    def _add_output_script(self, script):
        ...
        script.add_action(OUTPUT, self.DRAW_HELICOPTER_ACTION)
    ...
    def _add_update_script(self, script):
        ...
        script.add_action(UPDATE, self.MOVE_HELICOPTER_ACTION)
        script.add_action(UPDATE, self.COLLIDE_BORDERS_ACTION)
        script.add_action(UPDATE, self.COLLIDE_BRICKS_ACTION)
        script.add_action(UPDATE, self.COLLIDE_HELICOPTER_ACTION)
        script.add_action(UPDATE, self.MOVE_HELICOPTER_ACTION)
